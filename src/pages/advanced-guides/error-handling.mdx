...
For a complete list of error codes and their meanings, refer to the [psychoroid.com API documentation](https://docs.psychoroid.comafrica/api-reference/errors).
...
```javascript
try {
  const response = await axios.post('https://api.psychoroid.comafrica/v1/payments', {
