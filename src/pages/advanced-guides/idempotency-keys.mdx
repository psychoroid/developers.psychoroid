...
```bash
curl -X POST \
  https://api.lomi.africa/v1/payments \
...
If you make multiple requests with the same idempotency key, lomi. will process the request only once and return the same response for subsequent requests.
