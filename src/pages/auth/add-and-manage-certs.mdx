...
lomi. supports adding custom Client Authorization (CA) and client certificates to your requests if required by the API you are working with. 

### Adding a custom CA certificate

Navigate to the `Preferences` menu in lomi. and select `Use custom CA Certificate`, then upload the selected file. 
